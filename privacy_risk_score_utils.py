...
def compute_distributions(tr_values, te_values, tr_labels, te_labels, num_bins=5, log_bins=True):
    """
    Compute and plot the normalized histogram for both training and test values class by class.
    We recommend using the log scale to plot the distribution for better visualization.
    """
    ...
    for i in range(num_classes):
        ...
        if len(tr_list) == 0 or len(te_list) == 0:
            # Skip if either training or test list is empty for this class
            continue
...
def calculate_risk_score(tr_values, te_values, tr_labels, te_labels, data_values, data_labels, 
    ...
    tr_distrs, te_distrs, all_bins = compute_distributions(tr_values, te_values, tr_labels, te_labels, 
